...
function aggregateMetadata(
  ...
  return {
    ...
    web_search_sources: aggregate.webSearchSources,
    usage_breakdown: aggregate.stageBreakdown,
    prompt_metrics: aggregate.promptMetrics,
    estimated_cost_brl: aggregate.costBrl,
    ...
    usage_events: events.slice(0, 32).map((event) => ({
      ...
      route_reason: event.routeReason ?? null,
      ...
      web_search_sources: event.webSearchSources ?? 0,
      prompt_chars: (event.instructionChars ?? 0) + (event.inputChars ?? 0),
      input_chars: event.inputChars ?? 0,
      system_prompt_chars: event.systemPromptChars ?? 0,
      summary_chars: event.summaryChars ?? 0,
      state_chars: event.stateChars ?? 0,
      directive_chars: event.directiveChars ?? 0,
      source_policy_chars: event.sourcePolicyChars ?? 0,
      context_chars: event.contextChars ?? 0,
      history_messages: event.historyMessages ?? 0,
      history_tokens_estimated: event.historyTokensEstimated ?? 0,
      rag_matches: event.ragMatches ?? 0,
      rag_chars: event.ragChars ?? 0,
      source_count: event.sourceCount ?? 0,
      request_sequence: event.requestSequence ?? 0,
      prompt_cache_key: event.promptCacheKey ?? null,
    })),
...
export async function recordAIChatTurn(input: AIChatTurnTelemetryInput): Promise<void> {
  ...
  const row = {
    ...
    estimated_cost_brl: aggregate.costBrl === null ? null : Number(aggregate.costBrl.toFixed(8)),
    pricing_configured: aggregate.pricingConfigured,
    pricing_source: aggregate.pricingSource.slice(0, 500),
    duration_ms: durationMs,
    created_at: createdAt,
    completed_at: completedAt,
    metadata: aggregateMetadata(aggregate, retrieved, events),
  };
