...
export function sourceDirective(evidence: EvidenceAssessment): string {
  if (evidence.requiresCurrentMarketSearch) {
    return `POLÍTICA DE FONTES DESTE TURNO — MERCADO ATUAL: a base interna não contém cotação recente para a combinação pedida. Pesquise obrigatoriamente ANTES de montar a resposta, nesta ordem temporal: hoje, ontem e anteontem; e nesta ordem geográfica: praça pedida, praça vizinha, indicador estadual e indicador nacional. Priorize CEPEA/ESALQ, IEA, B3, Scot Consultoria, Conab e outras fontes de mercado identificáveis. Não trate registro histórico interno como preço atual. Também não aceite resultado sem data explícita como preço atual. Se não houver publicação nos três dias, use somente a fonte confiável mais recente encontrada, com data explícita e sem chamá-la de cotação de hoje. Toda cotação exige preço, unidade, praça, data e fonte. Entregue a referência mais recente já nesta primeira resposta; é proibido perguntar se o usuário quer que você a busque, consulte ou compare depois. Antes de enviar, descarte qualquer valor sem data ou fonte e continue a pesquisa. Nunca invente, estime ou use preço de memória. ${PECUARIA_AUTHORITY_POLICY}`;
  }
