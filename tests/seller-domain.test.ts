...
import { formatSellerList, matchSellerRequest, type PublicSeller } from "../src/lib/site/seller-domain.ts";

const sellers: PublicSeller[] = [
  { id: "1", name: "Ana Souza", role: "Consultora", region: "São José do Rio Preto", phone: null, whatsapp: "5517999991111" },
  { id: "2", name: "Bruno Lima", role: "Representante", region: "Monte Aprazível", phone: "1730002000", whatsapp: null },
];
...
test("pedido genérico retorna todos os vendedores individuais", () => {
  ...
  assert.deepEqual(result.sellers.map((seller) => seller.name), ["Ana Souza", "Bruno Lima"]);
  assert.match(formatSellerList(result), /Ana Souza/);
  assert.match(formatSellerList(result), /WhatsApp: 5517999991111/);
});
...
test("cidade conhecida filtra a região e entende o apelido Rio Preto", () => {
  ...
  assert.deepEqual(result.sellers.map((seller) => seller.name), ["Ana Souza"]);
});
...
test("não inventa contato ausente", () => {
  const result = matchSellerRequest("lista de vendedores", [{ ...sellers[0], phone: null, whatsapp: null }]);
