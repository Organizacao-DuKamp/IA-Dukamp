-- Torna a sincronização de mensagens proativas idempotente e recuperável.
ALTER TABLE public.whatsapp_proactive_queue
  ADD COLUMN IF NOT EXISTS history_synced_at TIMESTAMPTZ;

-- Envios anteriores podem já ter respostas posteriores no histórico.
-- Não os acrescente no fim fora de ordem.
